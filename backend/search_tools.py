...
class CourseOutlineTool(Tool):
    """Tool for retrieving course outline information from the metadata collection"""
    ...
    def get_tool_definition(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": "Retrieve the complete outline for a specific course including title, link, and all lessons",
            "parameters": {
                "type": "object",
                "properties": {
                    "course_title": {
                        "type": "string", 
                        "description": "Course title (partial matches work, e.g. 'MCP', 'Introduction', 'RAG')"
                    }
                },
    ...
    def execute(self, course_title: str) -> str:
        ...
        try:
            ...
            if lessons:
                outline_parts.append(f"**Number of Lessons:** {len(lessons)}")
                ...
                for lesson in sorted(lessons, key=lambda x: x.get('lesson_number', 0)):
                    ...
                    lesson_info = f"  {lesson_num}. {lesson_title}"
                    outline_parts.append(lesson_info)
            else:
